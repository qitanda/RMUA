...
#include "onnxparser/onnx_utils.hpp"
#include "onnxparser/common.hpp"
...
namespace TRT {
	...
	// 	shared_ptr<IBuilder> builder = common::infer_object(createInferBuilder(gLogger));
	// 	if (builder == nullptr) {
	...
	// 	shared_ptr<IBuilderConfig> config = common::infer_object(builder->createBuilderConfig());
	// 	if (mode == Mode::FP16) {
	...
	// 	shared_ptr<INetworkDefinition> network = common::infer_object(builder->createNetworkV2(explicitBatch));
	...
	// 		// network = common::infer_object(trt_builder->createNetworkV2(explicitBatch));
	...
	// 		shared_ptr<nvonnxparser::IParser> onnxParser = common::infer_object(nvonnxparser::createParser(*network, gLogger));
	...
	// 	shared_ptr<ICudaEngine> engine = common::infer_object(builder->buildEngineWithConfig(*network.get(), *config.get()));
	// 	if (engine == nullptr) {
    ...
    //                           severity == Severity::kINFO           ? "   INFO" :
		common::TRT_Logger trt_logger(nvinfer1::ILogger::Severity::kWARNING);
		auto trt_builder = common::infer_object(nvinfer1::createInferBuilder(trt_logger));
		auto trt_network = common::infer_object(trt_builder->createNetworkV2(explicit_batch));
		auto trt_parser  = common::infer_object(nvonnxparser::createParser(*trt_network, trt_logger));

		if(trt_builder == nullptr) {
			INFOE("Can not create trt_builder.");
			return false;
		}
		if(trt_parser == nullptr) {
			INFOE("Can not create trt_parser.");
			return false;
		}

		bool fp16 = trt_builder->platformHasFastFp16();
		INFO("Building TensorRT engine, FP16 available: %d", (int)fp16);

		auto builder_config = common::infer_object(trt_builder->createBuilderConfig());
    builder_config->setMaxWorkspaceSize(maxWorkspaceSize);
    if(fp16 && mode == Mode::FP16)
      builder_config->setFlag(nvinfer1::BuilderFlag::kFP16);
		...
		if(source.type() == ModelSourceType::OnnX){
			...
			if (!trt_parser->parseFromFile(source.onnxmodel().c_str(), 1)) {
				INFOE("Can not parse OnnX file: %s", source.onnxmodel().c_str());
				return false;
			}
		}
		...
		set_layer_hook_reshape(nullptr);
		auto inputTensor = trt_network->getInput(0);
		auto inputDims = inputTensor->getDimensions();
		...
		int net_num_input = trt_network->getNbInputs();
		INFO("Network has %d inputs:", net_num_input);
		vector<string> input_names(net_num_input);
		for(int i = 0; i < net_num_input; ++i){
			auto tensor = trt_network->getInput(i);
			auto dims = tensor->getDimensions();
			auto dims_str = join_dims(vector<int>(dims.d, dims.d+dims.nbDims));
			INFO("      %d.[%s] shape is %s", i, tensor->getName(), dims_str.c_str());
		...
		int net_num_output = trt_network->getNbOutputs();
		INFO("Network has %d outputs:", net_num_output);
		for(int i = 0; i < net_num_output; ++i){
			auto tensor = trt_network->getOutput(i);
			auto dims = tensor->getDimensions();
			auto dims_str = join_dims(vector<int>(dims.d, dims.d+dims.nbDims));
			INFO("      %d.[%s] shape is %s", i, tensor->getName(), dims_str.c_str());
		}

		int net_num_layers = trt_network->getNbLayers();
		INFO("Network has %d layers:", net_num_layers);
		for(int i = 0; i < net_num_layers; ++i){
		...
		INFO("Building engine...");
		auto time_start = iLogger::timestamp_now();
		shared_ptr<ICudaEngine> engine(trt_builder->buildEngineWithConfig(*trt_network, *builder_config), destroy_nvidia_pointer<ICudaEngine>);
		if (engine == nullptr) {
			INFOE("engine is nullptr");
			return false;
		}
