...
void pre_lab(const cv::Mat inimage, cv::Mat &outimage)
{
	cv::Mat blurimg,blur1_img,blur2_img,Labimg, thrimg0,
		thrimg,thrimg2,thrimg3,eroimg,diaimg,blur3_img,blur4_img, 
		blue_L, blue_b,blue,blue_erode,blue_dilate,blue_blur,hsvImage,tr_image,helpimage;
	//以下是图像预处理
	...
	cv::split(Labimg,lab);
	cv::threshold(lab[0],thrimg0,155,255,CV_THRESH_BINARY);
    ...
    cv::transpose(hsv[2], tr_image);  // 将图像进行转置
    cv::flip(tr_image,helpimage , 1);    // 1表示垂直翻转
...
//     }
	...
	//debug 展示功能
    outimage = hsv[2];
cv::imshow("Lab[0]",lab[0]);
cv::imshow("hsv[0]",hsv[0]);
cv::imshow("hsv[2]_flip",helpimage);
cv::imshow("hsv[2]",hsv[2]);
cv::imshow("thrimg",thrimg0);
	cv::waitKey(5);
}
...
int main(int argc, char** argv) {
    ...
    aamedl.SetParameters(CV_PI / 3, 3.4, 0.65); 
    aamedr.SetParameters(CV_PI / 3, 3.4, 0.65); 
	while(ros::ok())
	{	if (!left_img.empty())
