...
public:
    ...
    void Init(ros::ServiceClient *srv, UAVMode_e *mod) {
        stamp_ = ros::Time::now();
        srv_ = srv;
        mode = mod;
        takeoff_flag = false;
    ...
    void Callback(const mavros_msgs::RCIn::ConstPtr &msg) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if(fabs(ch[i]) < 0.15) ch[i] = 0;
        }
        
        if (ch[5] < -0.5 && reboot_ch_last > 0.5) { // reboot PX4
        ...
        if (fabs(ch[4] - mode_ch_last) > 0.05) mode_triggle_flag = true;
        else mode_triggle_flag = false;
        if ((ch[4] > -0.6 && ch[4] < -0.3) && mode_triggle_flag) *mode = Command;
        else if (ch[4] < -0.7 && mode_triggle_flag) {
            *mode = Hover;
            if (ch[10] < -0.5) {
                *mode = Takeoff;
                takeoff_flag = true;
            ...
            if (mode_triggle_flag) *mode = Manual;
        }
        mode_ch_last = ch[4];

        if (fabs(ch[10] - takeoff_ch_last) > 0.05 && ch[10] < -0.2) takeoff_triggle_flag = true;
        else takeoff_triggle_flag = false;
        if ((*mode == Hover || *mode == Takeoff || *mode == Land) && ch[10] < -0.2 && takeoff_triggle_flag) {
    ...
    double ch[12];
    double reboot_ch_last, mode_ch_last, takeoff_ch_last;
    UAVMode_e *mode;
    bool mode_triggle_flag, takeoff_triggle_flag, takeoff_flag;
...
private:
    ros::Subscriber odom_sub_, state_sub_, imu_sub_, rc_sub_, battery_sub_, cmd_sub_;
