...
BasicDev::BasicDev(ros::NodeHandle *nh)
{
    ...
    img_circle_suber = nh->subscribe<airsim_ros::Circle>("TOPICNAME", 1, std::bind(&BasicDev::img_circle_cb, this, std::placeholders::_1));
    ...
    mode_pub = nh->advertise<flag_msgs::mode_flag>("mode_flag", 1);
    ...
    windmill.pose.position.x = -43;
    windmill.pose.position.y = 108;
    windmill.pose.position.z = -28;
    // Only for debug
    last_check_index = -1;
    ...
    while (ros::ok())
    {
        ...
        else
        {
            ...
            if (path_change_state)
            {
                ...
                count_pub.publish(target_ID); // 如果感知为空可能被空加
...
void BasicDev::img_circle_cb(const airsim_ros::Circle::ConstPtr &msg)
{
    ...
    if ((abs(last_img_circle_pt.position.x - img_circle_pt.position.x) > 0.05) || (abs(last_img_circle_pt.position.y - img_circle_pt.position.y) > 0.05) || (abs(last_img_circle_pt.position.z - img_circle_pt.position.z) > 0.05) || (abs(last_img_circle_pt.yaw - img_circle_pt.yaw) > 5))
    {
...
nav_msgs::Path BasicDev::get_1to3_path(const airsim_ros::Circle circle_position)
{
    ...
    float distant_1 = 5, distant_2 = 0.4, distant_3 = 6;
    // circle_yaw -= M_PI / 2;
    // if (circle_yaw > M_PI)
    //     circle_yaw -= 2 * M_PI;
    ...
    // else if ((pow((position_1.pose.position.x - origin_circle_pt.poses[target_ID.data + 1].position.x), 2) +
    // pow((position_1.pose.position.y - origin_circle_pt.poses[target_ID.data + 1].position.y), 2) +
    // pow((position_1.pose.position.z - origin_circle_pt.poses[target_ID.data + 1].position.z), 2)) >
    // (pow((position_6.pose.position.x - origin_circle_pt.poses[target_ID.data + 1].position.x), 2) +
    // pow((position_6.pose.position.y - origin_circle_pt.poses[target_ID.data + 1].position.y), 2) +
    // pow((position_6.pose.position.z - origin_circle_pt.poses[target_ID.data + 1].position.z), 2)))
    // {
    //     // path_3.poses.push_back(position_1);
    //     // path_3.poses.push_back(position_2);
    //     path_3.poses.push_back(position_3);
...
nav_msgs::Path BasicDev::get_path(airsim_ros::Circle made_path_circle_0, airsim_ros::Circle made_path_circle_1)
{
    ...
    if (target_ID.data == 12 || target_ID.data == 4)
    {
    ...
    //                     fron_1to3_path.poses[fron39055_1to3_path.poses.size() - 1].pose.position.z}; // 前圈后点
...
bool BasicDev::JudgeCircleThroughState()
{
    ...
    Eigen::Vector3d des_p(origin_circle_pt.poses[target_ID.data].position.x, origin_circle_pt.poses[target_ID.data].position.y, origin_circle_pt.poses[target_ID.data].position.z);
...
void BasicDev::YawPublish()
{
    ...
    double target_yaw = gt_circle_pt.poses[target_ID.data].yaw / 180.0 * M_PI;
    double current_yaw = tf::getYaw(gt_pose.pose.orientation);
