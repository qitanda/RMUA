...
float measure_noise=0.01;
float process_noise=0.01;
KalmanCV KF_x,KF_y,KF_z;

ros::Publisher gt_odom_pub;
Eigen::Vector3d gt_p,gt_p_last,gt_v;
...
void SimulatorPoseCallback(const geometry_msgs::PoseStamped::ConstPtr msg)
{
    ...
    else
    {
        ...
        double dt = time_quene[time_quene.size()-1]-time_quene[time_quene.size()-2];
        if(first_flag)
        {
            first_flag=false;
            Eigen::Vector2d Vx(gt_v[0],0);
            Eigen::Vector2d Vy(gt_v[1],0);
            Eigen::Vector2d Vz(gt_v[2],0);
            KF_x.Reset(Vx);
            KF_y.Reset(Vy);
            KF_z.Reset(Vz);
        }
        ...
        {
            KF_x.MeasureUpdate(gt_v[0],dt);
            KF_y.MeasureUpdate(gt_v[1],dt);
            KF_z.MeasureUpdate(gt_v[2],dt);
        }
        std::cout<<"gtv:"<<gt_v[0]<<" "<<gt_v[1]<<" "<<gt_v[2]<<std::endl;
        gt_v[0]=KF_x.X_(0);
        gt_v[1]=KF_y.X_(0);
        gt_v[2]=KF_z.X_(0);
        std::cout<<"kfv:"<<gt_v[0]<<" "<<gt_v[1]<<" "<<gt_v[2]<<std::endl;
